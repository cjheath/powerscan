...
int main(int argc, char **argv)
{
	...
	for (int repetition = 0; pc->web_port || pc->repetition_limit == 0 || repetition < pc->repetition_limit; repetition++)
...
//		"\t-a name\t\tSelect antenna\n"
		...
		"\t-w [port]\tRun continuously as a local  web server on this port (default 3909)\n"
		"\t-h\t\tThis help message\n"
	);
	exit(exit_code);
}
...
bool gather_parameters(ProgramConfiguration* pc, int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "vd:C:a:g:s:e:r:c:1l:t:w::h?")) != -1) {
		switch (opt) {
		...
		case 'w':
			pc->web_port = optarg ? atol(optarg) : 3909;
			break;
